package com.devscious.quickstarthue.quickstart;
...
import com.devscious.quickstarthue.R;
import com.devscious.quickstarthue.data.AccessPointListAdapter;
import com.devscious.quickstarthue.data.HueSharedPreferences;
 ...
 * displayed (and clicking one of them shows the PushLink dialog allowing authentication).
 ...
 * When connected the MyApplicationActivity Activity is started.  This is where you should start
 * implementing your Hue App!  Have fun!
 *
...
public class PHHomeActivity extends Activity implements OnItemClickListener {
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    if (lastIpAddress != null && !lastIpAddress.equals("")) {
      ...
      if (!phHueSDK.isAccessPointConnected(lastAccessPoint)) {
        PHWizardAlertDialog.getInstance()
            .showProgressDialog(R.string.connecting, PHHomeActivity.this);
  ...
  private PHSDKListener listener = new PHSDKListener() {
    ...
    @Override public void onAccessPointsFound(List<PHAccessPoint> accessPoint) {
    ...
    @Override public void onCacheUpdated(List<Integer> arg0, PHBridge bridge) {
      Log.w(TAG, "On CacheUpdated");
    }

    @Override public void onBridgeConnected(PHBridge b, String username) {
      phHueSDK.setSelectedBridge(b);
      phHueSDK.enableHeartbeat(b, PHHueSDK.HB_INTERVAL);
      phHueSDK.getLastHeartbeat()
          .put(b.getResourceCache().getBridgeConfiguration().getIpAddress(),
    ...
    @Override public void onConnectionResumed(PHBridge bridge) {
      ...
      Log.v(TAG, "onConnectionResumed" + bridge.getResourceCache()
          .getBridgeConfiguration()
          .getIpAddress());
      phHueSDK.getLastHeartbeat()
          .put(bridge.getResourceCache().getBridgeConfiguration().getIpAddress(),
              System.currentTimeMillis());
      ...
      if (code == PHHueError.NO_CONNECTION) {
        ...
        if (!lastSearchWasIPScan) {  // Perform an IP Scan (backup mechanism) if UPNP and Portal Search fails.
          ...
          PHBridgeSearchManager sm =
              (PHBridgeSearchManager) phHueSDK.getSDKService(PHHueSDK.SEARCH_BRIDGE);
    ...
    @Override public void onParsingErrors(List<PHHueParsingError> parsingErrorsList) {
  ...
  public void doBridgeSearch() {
    PHWizardAlertDialog.getInstance()
        .showProgressDialog(R.string.search_progress, PHHomeActivity.this);
    PHBridgeSearchManager sm =
        (PHBridgeSearchManager) phHueSDK.getSDKService(PHHueSDK.SEARCH_BRIDGE);
  ...
  public void startMainActivity() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
      intent.addFlags(
          0x8000); // equal to Intent.FLAG_ACTIVITY_CLEAR_TASK which is only available from API level 11
    }
